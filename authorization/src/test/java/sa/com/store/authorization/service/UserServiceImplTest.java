...
class UserServiceImplTest {
    ...
    void disableUser_Success() {
        // Arrange
        Long userId = 1L;
        UserEntity userToDisable = UserEntity.builder()
                .userId(2L)
                .username("otheruser")
        ...
        // Act
        assertDoesNotThrow(() -> userService.disableUser(userId));
        ...
        ArgumentCaptor<UserEntity> entityCaptor = ArgumentCaptor.forClass(UserEntity.class);
        verify(userRepository).save(entityCaptor.capture());
        assertFalse(entityCaptor.getValue().isEnabled());
    }
    ...
    @Test
    void disableUser_CannotDisableOwnAccount() {
        // Arrange
        Long userId = 1L;
        ...
        assertEquals("You cannot disable your own account", exception.getMessage());
        verify(userRepository, never()).save(any());
    }
    ...
    void enableUser_Success() {
        ...
        when(userRepository.save(any(UserEntity.class))).thenReturn(disabledUser);

        // Act
        assertDoesNotThrow(() -> userService.enableUser(userId));
        ...
        ArgumentCaptor<UserEntity> entityCaptor = ArgumentCaptor.forClass(UserEntity.class);
        verify(userRepository).save(entityCaptor.capture());
        assertTrue(entityCaptor.getValue().isEnabled());
    }
